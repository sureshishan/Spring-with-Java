 ...
 * checked exception raising statement in method. But in case of catch it not
 ...
 * 
 * objects from one system to another system will transfered through only with
 * files.
 * 
 ...
 * 
 * all the wrapper classes as well as Strings are by default Serializable in
 * Java
 * 
 * When there is a significant difference/ change in serializable members of the
 * class( in the serializable class) to represent the difference that is being
 * changed we use an attribute i.e serialVersionUID it tells the specific
 * vertion of the serializable class.
 * 
 * private static final long serialVersionUID = 1L;
 * 
 * when there is a significant change we should change the serialVersonUID=2L;
 * 
 * 
 * Externalization:
 * 
 * persisting the object into the file system using Externalizable interface
 ...
 * In serialization
 * 
 * the class information will persisted as well as the attributes information
 * will be persisted as well as the values of the corresponding attributes will
 * also be persisted
 * 
 * but in externalized file class info does not exist attribute info does not
 * exist only values would be present and the corresponding data type
 * 
 * the extention of the file will be same (.ser)
 * 
 * so the externalizable file will be of less size when compare to ser file.
 * 
 * Externalizable is sub interface to Serializable
 * 
 * and externalizable is not a marker interface
 * 
 * it has 2 methods 
 * 
 * 1) writeExternal();
 * 2) readExternal();
 * 
 * while performing deexternalization we need to have default construcotor
 */
